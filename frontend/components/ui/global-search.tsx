...
import { Search, Award, Briefcase, FileText, ArrowRight, X, Loader2 } from "lucide-react";
...
export function GlobalSearch() {
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const [credsRes, jobsRes] = await Promise.allSettled([
        api.get(`/credentials?search=${encodeURIComponent(q)}&limit=4`),
        api.get(`/recommendations/jobs/relevant?limit=4`),
      ]);

      const items: SearchResult[] = [];

      if (credsRes.status === "fulfilled") {
        (credsRes.value.data.credentials || []).slice(0, 4).forEach((c: any) => {
          items.push({
            id: c._id,
            type: "credential",
            title: c.title,
            subtitle: c.issuerName || "Credential",
            href: "/credentials",
          });
        });
      }
      ...
      if (jobsRes.status === "fulfilled") {
        (jobsRes.value.data.jobs || [])
          .filter((j: any) => j.title.toLowerCase().includes(q.toLowerCase()))
          .slice(0, 3)
          .forEach((j: any) => {
            items.push({
              id: j._id,
              type: "job",
              title: j.title,
              subtitle: j.employerId?.companyName || "Job",
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <input
                    ...
                    placeholder="Search credentials, skills, jobs..."
                ...
                <div className="max-h-[360px] overflow-y-auto p-2">
                  ...
                  {displayItems.map((item, i) => (
                    ...
                    >
                      <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center shrink-0", getIconBg(item.type))}>
                        {getIcon(item.type)}
                      </div>
